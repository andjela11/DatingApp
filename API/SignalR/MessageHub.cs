...
namespace API.SignalR
{
    ...
    public class MessageHub : Hub
    {
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            var grpName = GetGroupName(Context.User.GetUsername(), otherUser);

            await Groups.AddToGroupAsync(Context.ConnectionId, grpName);
            var group = await AddToGroup(grpName);

            await Clients.Group(grpName).SendAsync("UpdatedGroup", group);
        ...
        public async Task SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            if(username == createMessageDto.RecipientUsername.ToLower())
                throw new HubException("You cannot send messages to ypurself");
            ...
            {
                await Clients.Group(groupName).SendAsync("NewMessage", _mapper.Map<MessageDto>(msg)); 
            }    
        }
